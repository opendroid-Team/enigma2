...
class Swap(Screen):
	skin = """
	<screen name="Swap" position="center,center" size="700,500" title="Swap File Manager" flags="wfBorder" >
		...
		<widget name="autostart_off" position="10,50" zPosition="1" pixmap="skin_default/installed.png/lock_off.png" size="32,32" alphatest="on" />
		<widget name="autostart_on" position="10,50" zPosition="2" pixmap="skin_default/installed.png/lock_on.png" size="32,32" alphatest="on" />
	...
	def getSwapDevice(self):
		...
		if path.exists('/etc/rcS.d/S98SwapManager'):
			...
			config.OPD_panel.swapautostart.value = True
			config.OPD_panel.swapautostart.save()
	...
	def updateSwap2(self, result = None, retval = None, extra_args = None):
		...
		if config.OPD_panel.swapautostart.value and self.swap_place:
			self['autostart_off'].hide()
			self['autostart_on'].show()
		else:
			config.OPD_panel.swapautostart.value = False
			config.OPD_panel.swapautostart.save()
			configfile.save()
			self['autostart_on'].hide()
			self['autostart_off'].show()
