...
# modified to suit my needs, by j,puig 13-1-2015
...
class MyServiceInfoEX(Poll, Converter, object):
	...
	@cached
	def getText(self):
		self.stream = { 'apid':"---", 'vpid':"---", 'sid':"---", 'onid':"---", 'tsid':"---", 'pcrpid':"---", 'caids':"FTA", 'pmtpid':"N/A", 'txtpid':"N/A", 'xres':"", 'yres':"", 'atype':"", 'vtype':"", 'avtype':"", 'fps':"", 'tbps':"",}
		streaminfo = ""
		array_caids = []
		...
		if self.getServiceInfoString(info, iServiceInformation.sAudioPID) != "N/A":
			self.stream['apid'] = "%04d" % int(self.getServiceInfoString(info, iServiceInformation.sAudioPID)) \
			+ "  (0x%0.4X)"% int(self.getServiceInfoString(info, iServiceInformation.sAudioPID)) 
		if self.getServiceInfoString(info, iServiceInformation.sVideoPID) != "N/A":
			self.stream['vpid'] = "%04d" % int(self.getServiceInfoString(info, iServiceInformation.sVideoPID)) \
			+ "  (0x%0.4X)"% int(self.getServiceInfoString(info, iServiceInformation.sVideoPID))
		if self.getServiceInfoString(info, iServiceInformation.sSID) != "N/A":
			self.stream['sid'] = "%04d" % int(self.getServiceInfoString(info, iServiceInformation.sSID)) \
			+ "  (0x%0.4X)"% int(self.getServiceInfoString(info, iServiceInformation.sSID))
		if self.getServiceInfoString(info, iServiceInformation.sONID) != "N/A":
			self.stream['onid'] = "%04d" % int(self.getServiceInfoString(info, iServiceInformation.sONID)) \
			+ "  (0x%0.4X)"% int(self.getServiceInfoString(info, iServiceInformation.sONID))
		if self.getServiceInfoString(info, iServiceInformation.sTSID) != "N/A":
			self.stream['tsid'] = "%04d" % int(self.getServiceInfoString(info, iServiceInformation.sTSID)) \
			+ "  (0x%0.4X)"% int(self.getServiceInfoString(info, iServiceInformation.sTSID))
		if self.getServiceInfoString(info, iServiceInformation.sPCRPID) != "N/A":
			self.stream['pcrpid'] = "%04d" % int(self.getServiceInfoString(info, iServiceInformation.sPCRPID)) \
			+ "  (0x%0.4X)"% int(self.getServiceInfoString(info, iServiceInformation.sPCRPID))
		if self.getServiceInfoString(info, iServiceInformation.sPMTPID) != "N/A":
			self.stream['pmtpid'] = "%04d" % int(self.getServiceInfoString(info, iServiceInformation.sPMTPID)) \
			+ "  (0x%0.4X)"% int(self.getServiceInfoString(info, iServiceInformation.sPMTPID))
		if self.getServiceInfoString(info, iServiceInformation.sTXTPID) != "N/A":
			self.stream['txtpid'] = "%04d" % int(self.getServiceInfoString(info, iServiceInformation.sTXTPID)) \
			+ "  (0x%0.4X)"% int(self.getServiceInfoString(info, iServiceInformation.sTXTPID))
		...
		self.stream['vtype'] = ("MPEG 2", "MPEG 4", "MPEG 1", "MPEG4-II", "VC1", "VC1-SM", "")[info.getInfo(iServiceInformation.sVideoType)]
		self.stream['avtype'] = ("MPEG 2", "MPEG 4", "MPEG 1", "MPEG4-II", "VC1", "VC1-SM", "")[info.getInfo(iServiceInformation.sVideoType)] + "\c00?25=41" + " / " + "\c0078:0=7" + self.stream['atype']
