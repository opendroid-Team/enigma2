...
class EPGSelection(Screen, HelpableScreen):
    ...
    def __init__(self, session, service = None, zapFunc = None, eventid = None, bouquetChangeCB = None, serviceChangeCB = None, EPGtype = None, StartBouquet = None, StartRef = None, bouquets = None):
        ...
        self['dialogactions'] = HelpableActionMap(self, 'WizardActions', {'back': (self.closeChoiceBoxDialog, _('Close dialog'))}, -1)
        self['dialogactions'].csel = self
        self['dialogactions'].setEnabled(False)
        self['okactions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (self.closeScreen, _('Exit EPG')),
         'OK': (self.OK, _('Zap to channel (setup in menu)')),
         'OKLong': (self.OKLong, _('Zap to channel and close (setup in menu)'))}, -1)
        self['okactions'].csel = self
        self['colouractions'] = HelpableActionMap(self, 'ColorActions', {'red': (self.redButtonPressed, _('IMDB search for current event')),
         ...
         'bluelong': (self.blueButtonPressedLong, _('Show AutoTimer List'))}, -1)
        self['colouractions'].csel = self
        self['recordingactions'] = HelpableActionMap(self, 'InfobarInstantRecord', {'ShortRecord': (self.recButtonPressed, _('Add a record timer for current event')),
         'LongRecord': (self.recButtonPressedLong, _('Add a zap timer for current event'))}, -1)
        self['recordingactions'].csel = self
        if self.type == EPG_TYPE_SIMILAR:
            self.currentService = service
            self.eventid = eventid
            self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'info': (self.Info, _('Show detailed event info')),
             'infolong': (self.InfoLong, _('Show single epg for current channel')),
             'menu': (self.createSetup, _('Setup menu'))}, -1)
            self['epgactions'].csel = self
        elif self.type == EPG_TYPE_SINGLE:
            self.currentService = ServiceReference(service)
            self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'nextService': (self.nextService, _('Goto next channel')),
             'prevService': (self.prevService, _('Goto previous channel')),
             'info': (self.Info, _('Show detailed event info')),
             'epg': (self.Info, _('Show detailed event info')),
             'menu': (self.createSetup, _('Setup menu'))}, -1)
            self['epgactions'].csel = self
            self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (self.prevPage, _('Move up a page')),
             'right': (self.nextPage, _('Move down a page')),
             'up': (self.moveUp, _('Go to previous channel')),
             'down': (self.moveDown, _('Go to next channel'))}, -1)
            ...
            if self.type == EPG_TYPE_INFOBAR:
                ...
                self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'nextBouquet': (self.nextBouquet, _('Go to next bouquet')),
                 ...
                 'menu': (self.createSetup, _('Setup menu'))}, -1)
                self['epgactions'].csel = self
                self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (self.prevService, _('Go to previous channel')),
                ...
                self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (self.prevPage, _('Move up a page')),
                 ...
                 'down': (self.moveDown, _('Go to next channel'))}, -1)
                self['epgcursoractions'].csel = self
            self['input_actions'] = HelpableNumberActionMap(self, 'NumberActions', {'0': (self.keyNumberGlobal, _('enter number to jump to channel.')),
             ...
             '9': (self.keyNumberGlobal, _('enter number to jump to channel.'))}, -1)
            ...
            self['bouquetokactions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (self.BouquetlistHide, _('Close bouquet list.')),
             'OK': (self.BouquetOK, _('Change to bouquet'))}, -1)
            self['bouquetokactions'].csel = self
            self['bouquetokactions'].setEnabled(False)
            self['bouquetcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (self.moveBouquetPageUp, _('Go to previous event')),
             'right': (self.moveBouquetPageDown, _('Go to next event')),
             'up': (self.moveBouquetUp, _('Go to previous channel')),
             'down': (self.moveBouquetDown, _('Go to next channel'))}, -1)
            self['bouquetcursoractions'].csel = self
            self['bouquetcursoractions'].setEnabled(False)
            self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (self.leftPressed, _('Go to previous event')),
             'right': (self.rightPressed, _('Go to next event')),
             'up': (self.moveUp, _('Go to previous channel')),
             'down': (self.moveDown, _('Go to next channel'))}, -1)
            self['epgcursoractions'].csel = self
            self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'nextService': (self.nextService, _('Jump forward 24 hours')),
             ...
             'menu': (self.createSetup, _('Setup menu'))}, -1)
            self['epgactions'].csel = self
            self['input_actions'] = HelpableNumberActionMap(self, 'NumberActions', {'1': (self.keyNumberGlobal, _('Reduce time scale')),
             ...
             '0': (self.keyNumberGlobal, _('Move to home of list'))}, -1)
            self['input_actions'].csel = self
            ...
            self['date'] = Label()
            self.bouquetlist_active = False
            self['bouquetokactions'] = HelpableActionMap(self, 'OkCancelActions', {'OK': (self.BouquetOK, _('Change to bouquet'))}, -1)
            self['bouquetokactions'].csel = self
            self['bouquetokactions'].setEnabled(False)
            self['bouquetcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (self.moveBouquetPageUp, _('Go to previous event')),
             'right': (self.moveBouquetPageDown, _('Go to next event')),
            ...
            self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (self.leftPressed, _('Go to previous event')),
             ...
             'down': (self.moveDown, _('Go to next channel'))}, -1)
            self['epgcursoractions'].csel = self
            self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'nextService': (self.nextPage, _('Move down a page')),
             'prevService': (self.prevPage, _('Move up a page')),
             'nextBouquet': (self.nextBouquet, _('Go to next bouquet')),
             'prevBouquet': (self.prevBouquet, _('Go to previous bouquet')),
             'input_date_time': (self.enterDateTime, _('Go to specific data/time')),
        ...
        if event is None:
            ...
            for timer in self.session.nav.RecordTimer.timer_list:
                if timer.eit == eventid and ':'.join(timer.service_ref.ref.toString().split(':')[:11]) == refstr:
                    ...
                    menu = [(_('Delete timer'),
                      'CALLFUNC',
                      self.RemoveChoiceBoxCB,
                      cb_func1), (_('Edit timer'),
                      'CALLFUNC',
                      self.RemoveChoiceBoxCB,
                      cb_func2), (_('Disable timer'),
                      'CALLFUNC',
                      self.RemoveChoiceBoxCB,
                      cb_func3)]
                ...
                if not manual:
                    menu = [(_('Add Timer'),
                      'CALLFUNC',
                      self.ChoiceBoxCB,
                      self.doRecordTimer), (_('Add AutoTimer'),
                      'CALLFUNC',
                      self.ChoiceBoxCB,
        ...
        if self.type == EPG_TYPE_SINGLE:
            ...
            if self.session.nav.getCurrentlyPlayingServiceOrGroup() and self.StartRef and self.session.nav.getCurrentlyPlayingServiceOrGroup().toString() != self.StartRef.toString():
                if self.zapFunc and self.StartRef and self.StartBouquet:
                    if self.type == EPG_TYPE_GRAPH and config.epgselection.graph_preview_mode.value or self.type == EPG_TYPE_MULTI and config.epgselection.multi_preview_mode.value or self.type in (EPG_TYPE_INFOBAR, EPG_TYPE_INFOBARGRAPH) and config.epgselection.infobar_preview_mode.value in ('1', '2') or self.type == EPG_TYPE_ENHANCED and config.epgselection.enhanced_preview_mode.value:
        ...
        if self.type == EPG_TYPE_GRAPH:
            if number == 1:
                ...
                basetime = (basetime[0],
                 basetime[1],
                 basetime[2],
                 int(config.epgselection.graph_primetimehour.value),
                 int(config.epgselection.graph_primetimemins.value),
                 0,
                 basetime[6],
                 basetime[7],
                 basetime[8])
            ...
            if number == 1:
                ...
                basetime = (basetime[0],
                 basetime[1],
                 basetime[2],
                 int(config.epgselection.infobar_primetimehour.value),
                 int(config.epgselection.infobar_primetimemins.value),
                 0,
                 basetime[6],
                 basetime[7],
                 basetime[8])
