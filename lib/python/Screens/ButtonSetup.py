...
import os
ButtonSetupKeys = [(_('Red'), 'red', 'Infobar/openSingleServiceEPG'),
 ...
 (_('Blue long'), 'blue_long', 'Module/Plugins.Extensions.FileCommander.plugin/FileCommanderScreen'),
 (_('Info (EPG)'), 'info', 'Infobar/openEventView'),
 (_('Info (EPG) Long'), 'info_long', 'Infobar/showEventInfoPlugins/1'),
 (_('Epg/Guide'), 'epg', 'Infobar/openGraphEPG'),
 ...
 (_('Rec'), 'rec', 'Infobar/startInstantRecording'),
 (_('Teletext'), 'text', ''),
 (_('Subtitle'), 'subtitle', ''),
 (_('Menu'), 'mainMenu', ''),
 (_('List/PVR'), 'list', ''),
 (_('List/PVR') + ' ' + _('long'), 'list_long', ''),
 (_('Favorites'), 'favorites', ''),
 (_('Favorites') + ' ' + _('long'), 'favorites_long', ''),
 (_('File'), 'file', ''),
...
class ButtonSetup(Screen):
    ...
    def __init__(self, session, args = None):
        ...
        self.setTitle(_('Quick button setup'))
        self['key_red'] = Button(_('Exit'))
        self['key_green'] = Button(_('Mode'))
        self.list = []
        ...
        self.getFunctions()
        self['actions'] = ActionMap(['OkCancelActions', 'ColorActions', 'DirectionActions'], {'ok': self.keyOk,
         'cancel': self.close,
         'red': self.close,
         'up': self.keyUp,
         'down': self.keyDown,
...
class ButtonSetupSelect(Screen):
    ...
    def __init__(self, session, key, args = None):
        ...
        self['actions'] = ActionMap(['OkCancelActions',
         'ColorActions',
         'DirectionActions',
         'KeyboardInputActions'], {'ok': self.keyOk,
         'cancel': self.cancel,
         'red': self.cancel,
         'green': self.save,
         'up': self.keyUp,
    ...
    def keyOk(self):
        if self.mode == 'list':
            ...
            elif currentSelected[:2] in self.selected:
...
class InfoBarButtonSetup:
    ...
    def __init__(self):
        self['ButtonSetupButtonActions'] = helpableButtonSetupActionMap(self, 'ButtonSetupActions', dict(((x[1], (self.ButtonSetupGlobal, boundFunction(self.getHelpText, x[1]))) for x in ButtonSetupKeys)), -10)
        self.longkeyPressed = False
